...
@property (nonatomic) id <OnBreakControllerDelegate> delegate;
...
@property (nonatomic, weak) IBOutlet UILabel *breakStartedLabel;
...
@implementation OnBreakController
...
{
    [super viewDidLoad];
    [self.containerView setBackgroundColor:[self.theme onBreakBackgroundColor]];

    self.breakStartedLabel.text        = self.punch.breakType.name;
    self.breakStartedLabel.font        = [self.theme breakLabelFont];
    self.breakStartedLabel.textColor   = [self.theme breakLabelColor];
    ...
    [self.breakStartedLabel setAccessibilityIdentifier:@"punch_break_lbl"];
...
{
    [self.delegate onBreakControllerDidResumeWork:self];
}
...
- (void) updatePunchDurationLabel
{
    if (self.navigationController != nil && self.navigationController != (id)[NSNull null]) {
        ...
        [self.self.dayTimeSummaryController updateBreakHoursLabelWithOffset:durationComponents];
    }
    else{
        [self invalidateTimer];
    }
