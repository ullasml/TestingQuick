...
@interface HRSettingUpViewController () {
...
@end
@implementation HRSettingUpViewController
...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor=[Util colorWithHex:@"#275C91" alpha:1];
    ...
    self.pageImages = [NSArray arrayWithObjects:
                       [UIImage imageNamed:@"HRCarousel-1.png"],
                       [UIImage imageNamed:@"HRCarousel-2.png"],
                       [UIImage imageNamed:@"HRCarousel-3.png"],
                       [UIImage imageNamed:@"HRCarousel-4.png"],
                                             nil];
...
- (void)loadVisiblePages {
    ...
    NSInteger pagecontrolCurrentPage = self.pageControl.currentPage+1;
    if (self.pageControl.currentPage!=previousPage)
    {
        NSString *event= [NSString stringWithFormat:@"Carousel %d page",(int)pagecontrolCurrentPage];
...
//            if ([keychain storeUsername:[tempDict objectForKey:@"loginName"] password:[tempDict objectForKey:@"password"] companyName:[tempDict objectForKey:@"companyKey"] forService:@"repliconUserCredentials"]) {
//                NSLog(@"**SAVED**");
//            }
...
-(IBAction)startUsingRepliconClicked:(id)sender
{
    ...
    if ([keychain allCredentialsForService:@"repliconUserCredentials" limit:99] != nil && ![[keychain allCredentialsForService:@"repliconUserCredentials" limit:99] isKindOfClass:[NSNull class]]) {
        ...
        if (credentials != nil && ![credentials isKindOfClass:[NSNull class]]) {
            ...
            [defaults setObject:[NSString stringWithFormat:@"%@/%@",companyName,loginName] forKey:@"HRTECH_LOGIN"];
        }
    }
