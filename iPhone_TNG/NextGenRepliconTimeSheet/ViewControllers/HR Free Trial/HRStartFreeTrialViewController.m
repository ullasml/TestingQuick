//
//  HRStartFreeTrialViewController.m
...
#import "HRStartFreeTrialViewController.h"
...
@interface HRStartFreeTrialViewController () {
...
@implementation HRStartFreeTrialViewController
...
- (void)viewDidLoad {
    ...
    [Util setToolbarLabel:self withText: RPLocalizedString(HRFreeTrialTabbarTitle, @"") ];
    ...
    [self.signupButton setBackgroundImage:signUpStretchableImage forState:UIControlStateNormal];
    */
    ...
    [self.signupButton setTitle:RPLocalizedString(SignIn, @"") forState:UIControlStateNormal];
    
    float firstLineLabelHeight = [self getHeightForString: RPLocalizedString(@"By signing in you agree to Replicon's", @"") fontSize:14 forWidth:280];
    UILabel  *firstLineLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 280, firstLineLabelHeight)];
    firstLineLabel.text = RPLocalizedString(@"By signing in you agree to Replicon's", @"");
    ...
    self.textFieldCompany.placeholder = RPLocalizedString(@"Username", @"");
    self.textFieldEmail.placeholder = RPLocalizedString(@"Password", @"");
    self.textFieldEmail.secureTextEntry=YES;
    self.textFieldName.placeholder = RPLocalizedString(COMPANY_NAME_TEXT, @"");
    ...
    self.msgLabel.textColor=[Util colorWithHex:@"#333333" alpha:1];
    self.msgLabel.text=RPLocalizedString(HRTrialMsg, @"");
    [EventTracker.sharedInstance log:@"Free trial sign up page"];
    
    self.textFieldName.text=@"HRTech";
...
-(void) validateForm {
    NSString *passwordStr=@"HRTechPass2014";
    NSString *phoneNum=@"1-650-627-7695";
...
//                    [self validatePassword:passwordStr] &&
//                    _isCompanyValid && _isEmailValid
    ...
    self.textFieldName.returnKeyType =isValid ? UIReturnKeyNext : UIReturnKeyNext;
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if(textField.tag && textField.tag <= 2) {
    ...
    if(textField.tag == 2) {
        ...
        if(textField.text.length != 0) {
            ...
            else
            {
                ...
                [[RepliconServiceManager HrFreeTrialService] sendRequestToValidateCompanyServiceForString:self.textFieldCompany.text];
            }
        ...
        if([Util validateEmailAddress:self.textFieldEmail.text]) {
            ...
            else
            {
                ...
                // ******************************* IF USING REQUEST BUILDER *******************************
                [[RepliconServiceManager HrFreeTrialService] sendRequestToValidateEmailServiceForString:self.textFieldEmail.text];
            }
        ...
        // Password
        NSString *passwordStr=@"HRTechPass2014";
        self.validationImagePassword.image = [self validatePassword:passwordStr] ? _validIcon : _invalidIcon;
...
//        }
        ...
        NSMutableDictionary *dataDict = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                         self.textFieldName.text,@"company",
                                         self.textFieldCompany.text,@"username",
                                         self.textFieldEmail.text,@"password",
                                         nil];
        
       // [[RepliconServiceManager HrFreeTrialService] sendRequestTosignUpServiceForDataDict:dataDict];
        
        
        
        [[RepliconServiceManager HrFreeTrialService] sendrequestToFetchUserIntegrationDetailsWithDelegate:dataDict];
    }
