...
NSString *const ACKeychainCompanyName       = @"companyName";
...
- (NSDictionary *)credentialsFromKeychainItem:(NSDictionary *)item
{
    ...
    NSMutableDictionary *credentials = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                        ...
                                        identifier, ACKeychainCompanyName,
...
{
    return [self storeUsername:username password:password companyName:companyName expirationDate:nil forService:service];
}
...
{
    if ([self deleteAllCredentialsForService:service])
    {
        NSMutableDictionary *dictionary = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                           (__bridge id)kSecClassGenericPassword, (__bridge id)kSecClass,
                                           [username dataUsingEncoding:NSUTF8StringEncoding], (__bridge id)kSecAttrAccount,
                                           [companyName dataUsingEncoding:NSUTF8StringEncoding], (__bridge id)kSecAttrGeneric,
...
- (BOOL)storeUsername:(NSString *)username password:(NSString *)password companyName:(NSString *)companyName expirationDate:(NSDate *)expirationDate forService:(NSString *)service
{
    ...
    return [self storeUsername:username password:password companyName:companyName info:info forService:service];
}
