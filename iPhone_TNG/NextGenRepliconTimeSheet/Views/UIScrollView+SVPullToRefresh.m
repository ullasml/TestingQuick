...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:RPLocalizedString(@"Pull to refresh...", @""),
                                                       RPLocalizedString(@"Release to refresh...",@""),
                                                       RPLocalizedString(@"Loading...",@""),
                                                       nil];
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            ...
            case SVPullToRefreshStateLoading:
                ...
                CGRect minimumTextRect = [_titleLabel textRectForBounds:_titleLabel.bounds limitedToNumberOfLines:1];
                CGFloat loaderTextX = (self.bounds.size.width - minimumTextRect.size.width)/2 + (self.activityIndicatorView.frame.size.width) - 10;
                
                CGRect frame=_titleLabel.frame;
                frame.origin.x=loaderTextX;
                _titleLabel.frame=frame;
                
                frame=self.activityIndicatorView.frame;
                frame.origin.x=_titleLabel.frame.origin.x-frame.size.width-10;
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = RPLocalizedString(@"Pull to refresh...",);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    self.showsDateLabel = YES;
    self.dateLabel.text = [NSString stringWithFormat:RPLocalizedString(@"Last Updated: %@",), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:RPLocalizedString(@"Never",)];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:RPLocalizedString(@"Last Updated: %@",), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:RPLocalizedString(@"Never",)];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if([[[UIDevice currentDevice] systemVersion]newFloatValue] >= 5){
        ...
        NSUInteger numComponents = CGColorGetNumberOfComponents([self.arrowColor CGColor]);
        CGFloat colors[8];
