...
SPEC_BEGIN(PunchOutControllerSpec)

describe(@"PunchOutController", ^{
    __block PunchOutController <CedarDouble> *subject;
    __block id<PunchOutControllerDelegate> delegate;
    ...
    beforeEach(^{
        ...
        breakTypeRepository stub_method(@selector(fetchBreakTypesForUser:)).with(@"user-uri").and_return(breakTypeDeferred.promise);
        ...
        delegate = nice_fake_for(@protocol(PunchOutControllerDelegate));
        ...
        subject = [injector getInstance:[PunchOutController class]];
        ...
        [subject setupWithAddressControllerPresenter:addressControllerPresenter
                         serverDidFinishPunchPromise:serverDidFinishPunchDeferred.promise
                                            delegate:delegate
                                               punch:punch punchesPromise:punchesWithServerDidFinishPunchDeferred.promise];
    ...
    it(@"should show the correct punch state title", ^{
        [subject view];
        subject.punchStateLabel.text should equal(RPLocalizedString(@"Clocked In", @"Clocked In"));
    });

    describe(@"punching out", ^{
        beforeEach(^{
            [subject view]; // Force the viewDidLoad to be called.
    ...
    describe(@"when the view loads", ^{
        context(@"when location is not required", ^{
            ...
            it(@"should have the correct subviews wired", ^{
                ...
                scrollViewContentView.subviews should contain(subject.punchOutButton);
                scrollViewContentView.subviews should contain(subject.punchStateLabel);
                scrollViewContentView.subviews should contain(subject.punchDurationTimerLabel);
            });
        });
        ...
        context(@"when an address controller is present", ^{
            ...
            it(@"should have the correct subviews wired", ^{
                ...
                scrollViewContentView.subviews should contain(subject.punchStateLabel);
                scrollViewContentView.subviews should contain(subject.punchDurationTimerLabel);
                scrollViewContentView.subviews should contain(subject.addressLabelContainer);
            });
        });
    });
    ...
    describe(@"taking a break", ^{
        context(@"when the user is required to log their breaks", ^{
            ...
            context(@"when the user taps the take a break button", ^{
                ...
                context(@"when fetching the break type list succeeds", ^{
                    ...
                    context(@"when the user taps on a break type", ^{
                        ...
                        it(@"should tell its delegate that the user wants to take a break", ^{
                            BreakType *feelingsBreak = [[BreakType alloc] initWithName:@"Talk About Feelings Break" uri:@"feelings"];
                            delegate should have_received(@selector(punchOutControllerDidTakeBreakWithDate:breakType:)).with(tapDate, feelingsBreak);
                        });
                    });
