...
SPEC_BEGIN(ViewTimesheetNavigationControllerSpec)

describe(@"ViewTimesheetNavigationController", ^{
    __block ViewTimesheetNavigationController *subject;
    ...
    beforeEach(^{
        ...
        subject = [[ViewTimesheetNavigationController alloc] initWithRootViewController:rootViewController
        ...
        context(@"when notified that the network is offline", ^{
            ...
            context(@"when 10 seconds has elapsed", ^{
                ...
                it(@"should display the correct instructions", ^{
                    subject.offlineBanner.label.text should equal(@"No Internet Connection.");
                });
