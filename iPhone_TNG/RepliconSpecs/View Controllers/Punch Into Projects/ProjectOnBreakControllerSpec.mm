...
SPEC_BEGIN(ProjectOnBreakControllerSpec)

describe(@"ProjectOnBreakController", ^{
    __block ProjectOnBreakController <CedarDouble> *subject;
    __block id<Theme> theme;
    __block ButtonStylist *buttonStylist;
    __block LastPunchLabelTextPresenter *lastPunchLabelTextPresenter;
    __block id<ProjectOnBreakControllerDelegate> delegate;
    ...
    beforeEach(^{
        ...
        delegate = nice_fake_for(@protocol(ProjectOnBreakControllerDelegate));
    ...
    beforeEach(^{
        ...
        subject = [injector getInstance:[ProjectOnBreakController class]];
    ...
    describe(@"resuming work", ^{
        ...
        it(@"should notify its delegate", ^{
            delegate should have_received(@selector(projectonBreakControllerDidResumeWork:)).with(subject);
        });
    });
    ...
    describe(@"should show the punch attributes properly", ^{
        ...
        it(@"should show attributedString", ^{
            [subject.punchAttributesLabel.attributedText isEqualToAttributedString:expectedAttributedString];
        });
        
    });

    describe(@"resuming work when user has activity access", ^{
        beforeEach(^{
            userPermissionsStorage stub_method(@selector(hasActivityAccess)).and_return(YES);
            [subject view];
        });

        it(@"should disable the resume work button", ^{
            //subject.resumeWorkButton.userInteractionEnabled should be_falsy;
        });
    });
    ...
    describe(@"styling the view", ^{
        beforeEach(^{
            ...
            theme stub_method(@selector(transferButtonBackgroundColor)).and_return([UIColor greenColor]);
    ...
    describe(@"the view hierarchy", ^{
        ...
        it(@"should add the button to the scrollview's container view", ^{
            ...
            subject.containerView.subviews should contain(subject.cardContainerView);
            subject.cardContainerView.subviews should contain(subject.resumeWorkButton);
