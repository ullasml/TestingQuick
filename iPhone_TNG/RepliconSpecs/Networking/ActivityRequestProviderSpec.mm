...
SPEC_BEGIN(ActivityRequestProviderSpec)

describe(@"ActivityRequestProvider", ^{
    __block ActivityRequestProvider *subject;
    ...
    beforeEach(^{
        ...
        subject = [injector getInstance:[ActivityRequestProvider class]];
    ...
    context(@"requestForClientsForUserWithURI:searchText:page:", ^{
        ...
        context(@"When search text exists", ^{
            beforeEach(^{
                ...
                .with(@"GetPunchActivities").and_return(@"http://expected.endpoint/name");
                request = [subject requestForActivitiesForUserWithURI:@"some-user-uri" searchText:@"special-search-text" page:@2];
            });
        ...
        context(@"When search text not exists", ^{
            beforeEach(^{
                urlStringProvider stub_method(@selector(urlStringWithEndpointName:))
                .with(@"GetPunchActivities").and_return(@"http://expected.endpoint/name");
                request = [subject requestForActivitiesForUserWithURI:@"some-user-uri" searchText:nil page:@2];
            });
